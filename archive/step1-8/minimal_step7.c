// Step 7: x2APIC Support (MSR-based APIC access for KVM performance)
...
static void apic_init(void) {
    ...
    if (x2apic_available) {
        ...
        // In x2APIC mode, enable APIC via SVR MSR
        uint32_t svr = (uint32_t)rdmsr(X2APIC_SVR);
        wrmsr(X2APIC_SVR, svr | APIC_ENABLE);
...
void ap_entry(void) {
    ...
    // Enable x2APIC if BSP is using it
    if (use_x2apic) {
        uint64_t apic_msr = rdmsr(APIC_BASE_MSR);
