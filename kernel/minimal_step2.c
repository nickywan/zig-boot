// Step 2: ACPI Detection + Local APIC Init
...
// APIC MMIO (will be set after reading MSR)
...
static void apic_init(void) {
    ...
    // Set global APIC base pointer
    // Note: APIC is identity-mapped by default at 0xFEE00000
    apic_base = (volatile uint32_t*)apic_phys_addr;

    // Enable APIC in MSR (bit 11)
    if (!(apic_msr & (1 << 11))) {
        puts("[APIC] Enabling APIC in MSR...\n");
        wrmsr(APIC_BASE_MSR, apic_msr | (1 << 11));
    }

    // Software enable APIC (Spurious Vector Register)
...
void kernel_main(void) {
    ...
    puts("\n");
    puts("===========================================\n");
    puts("  Step 2: ACPI + Local APIC Init\n");
    puts("===========================================\n");
    puts("\n");
